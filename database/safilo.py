                        ...
                        if str(matched_product_from_database.id).strip() != '0':
                            ...
                            if str(scraped_product_by_brand.lens_color).strip():
                                ...
                                if str(scraped_product_by_brand.lens_color).strip().title() != str(matched_product_from_database.lens_color).strip().title():
                                    ...
                                    # if self.DEBUG: print('lens_color', matched_product_from_database.lens_color, scraped_product_by_brand.lens_color, matched_product_from_database.id)
                            else:
                                if str(scraped_product_by_brand.lens_code).strip():
                                    lens_color = query_processor.get_lens_color_against_code(str(scraped_product_by_brand.lens_code).strip().upper())
                                    if str(lens_color).strip().title() != str(matched_product_from_database.lens_color).strip().title():
                                        if lens_color:
                                            query_processor.update_product_lens_color(str(lens_color).strip().title(), matched_product_from_database.id)
                                            # if self.DEBUG: print('lens_color b', matched_product_from_database.lens_color, lens_color, matched_product_from_database.id)
                            ...
                            for scraped_variant_by_product in scraped_product_by_brand.variants:
                                ...
                                if str(matched_variant_from_database.id).strip() != '0':
                                    ...
                                    # check variant size #
                                    if str(scraped_variant_by_product.size).strip():
                            ...
                            if not str(scraped_product_by_brand.lens_color).strip() and str(scraped_product_by_brand.lens_code).strip():
                                scraped_product_by_brand.lens_color = query_processor.get_lens_color_against_code(scraped_product_by_brand.lens_code)
                                scraped_product_by_brand.lens_color = str(scraped_product_by_brand.lens_color).strip().title()
    ...
    def match_scraped_product_with_database_products(self, scraped_product_by_brand: Product, database_products_by_brand: list[Product]) -> Product:
        ...
        try:
            for database_product_by_brand in database_products_by_brand:
                ...
                scraped_product_lens_code = str(scraped_product_by_brand.lens_code).strip().upper().replace('/', '-')
                database_product_lens_code = str(database_product_by_brand.lens_code).strip().upper().replace('/', '-')
                if database_product_number == scraped_product_number and database_product_frame_code == scraped_product_frame_code and scraped_product_lens_code == database_product_lens_code:
