...
class Rudyproject_Database:
                ...
                if len(brand.products) > 0:
                    ...
                    for scraped_product_by_brand_index, scraped_product_by_brand in enumerate(brand.products):
                        ...
                        if str(matched_product_from_database.id).strip() != '0':
                            # check product name #
                            if str(scraped_product_by_brand.name).strip() and not str(matched_product_from_database.name).strip():
                                # when scraped_product name is not equal to database_product name
                                if str(scraped_product_by_brand.name).strip().title() != str(matched_product_from_database.name).strip().title():
                                    # update scraped_product name as new name
                                    query_processor.update_product_name(str(scraped_product_by_brand.name).strip().title(), matched_product_from_database.id)
                                    # if self.DEBUG: print('name', matched_product_from_database.name, scraped_product_by_brand.name)

                            # check product frame_color #
                            if not str(matched_product_from_database.frame_color).strip().title() and str(scraped_product_by_brand.frame_color).strip().title():
                            ...
                            if not str(matched_product_from_database.lens_color).strip().title() and str(scraped_product_by_brand.lens_color).strip().title():
                            ...
                            for scraped_variant_by_product in scraped_product_by_brand.variants:
                                ...
                                if str(matched_variant_from_database.id).strip() != '0':
                                    ...
                                    # check variant size #
                                    if not str(matched_variant_from_database.size).strip() and str(scraped_variant_by_product.size).strip():
    ...
    def match_scraped_product_with_database_products(self, scraped_product_by_brand: Product, database_products_by_brand: list[Product]) -> Product:
        ...
        try:
            for database_product_by_brand in database_products_by_brand:
                ...
                if database_product_number == scraped_product_number:
                    matched_product_from_database = database_product_by_brand
                    break
        except Exception as e:
            self.print_logs(f'Exception in match_scraped_product_with_database_products: {e}')
    ...
    def match_scraped_variant_with_database_variants(self, scraped_variant_by_product: Variant, database_variants_by_product: list[Variant]) -> Variant:
        ...
        try:
            ...
            self.print_logs(f'Exception in match_scraped_variant_with_database_variants: {e}')
    ...
    def get_img_360_string(self, img_360_urls: list[str]) -> str:
        ...
        try:
            ...
            self.print_logs(f'Exception in get_img_360_string: {e}')
