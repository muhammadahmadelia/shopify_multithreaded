                ...
                if len(brand.products) > 0:
                    ...
                    for scraped_product_by_brand_index, scraped_product_by_brand in enumerate(brand.products):
                        ...
                        if str(matched_product_from_database.id).strip() != '0':
                            ...
                            # check product frame_code #
                            if str(scraped_product_by_brand.frame_code).strip().upper() and not str(matched_product_from_database.frame_code).strip().upper():
                                # when scraped_product frame_code is not equal to database_product frame_code
                                if str(scraped_product_by_brand.frame_color).strip().title() != str(matched_product_from_database.frame_color).strip().title():
                                    # update scraped_product frame_code as new frame_code
                                    query_processor.update_product_frame_code(str(scraped_product_by_brand.frame_code).strip().upper(), matched_product_from_database.id)
